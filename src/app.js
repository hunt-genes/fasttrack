        ...
        if (q) {
            Result.find(query).limit(1000).sort("CHR_ID CHR_POS").lean().exec((err, results) => {
                ...
                if (download) {
                    ...
                    csv.writeToString(results, { headers: ["SNP_ID_CURRENT", "CHR_ID", "CHR_POS", "STRONGEST SNP-RISK ALLELE", "P-VALUE", "OR or BETA", "95% CI (TEXT)", "RISK ALLELE FREQUENCY", "INITIAL SAMPLE DESCRIPTION", "REPLICATION SAMPLE DESCRIPTION", "MAPPED_GENE", "hg19chr", "hg19pos", "DATE"], delimiter: "\t" }, (err, data) => {
                        res.set("Content-Type", "text/csv");
